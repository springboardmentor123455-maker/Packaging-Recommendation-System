...
# -------------------------------
df = pd.read_csv("/content/processed_materials.csv")

print("Dataset loaded successfully")
print(df.head())

# -------------------------------
# MODULE 3: ML DATA PREPARATION
# -------------------------------
...
print("✅ Module 3 completed: Feature selection, split, scaling ready")
...
print("\nCO2 IMPACT PREDICTION MODEL (XGBoost)")
...
# Final normalized rank
df['Material_Rank'] = 1 - (0.5 * df['Cost_Score'] + 0.5 * df['CO2_Score'])

# Sort and add integer rank position
df = df.sort_values(by='Material_Rank', ascending=False)
df['Material_Rank_Position'] = range(1, len(df) + 1)

# Save final CSV
df.to_csv("/content/ai_material_recommendations.csv", index=False)

print("\n✅ AI Material Recommendation System completed")
print("📁 Output saved as ai_material_recommendations.csv")
